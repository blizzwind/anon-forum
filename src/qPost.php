...
$SQL = "SELECT * FROM msg WHERE post_id=".$ID;
$RESULT = mysqli_query($CONN, $SQL);
if (mysqli_num_rows($RESULT) > 0) {
  while($ROW = mysqli_fetch_assoc($RESULT)) {
    $LIST .= "<div id='".$ROW["id"]."' class='post'><img src='".$ROW["ava"]."' onerror='reAva(this)'><div class='name'>".$ROW["name"]."</div><div class='time' data-time='".$ROW["created"]."'></div><a id='flag-".$ROW["id"]."' class='s-btn right' onclick='flagMsg(this)'><span class='icon-flag'></span></a><div class='txt'>".$ROW["text"]."</div></div>";
  }
} else {
  $LIST .= "";
}
